...
namespace rider::faiz
{
    ...
    struct iterator_traits_aux<Iter, true>
        : iterator_traits_impl<Iter,
              faiz::is_convertible_v<typename Iter::iterator_category,
                  input_iterator_tag> ||
                  is_convertible_v<typename Iter::iterator_category,
                      output_iterator_tag>>
    {
    };
    ...
    constexpr void advance(BiDirIter& i,
    ...
    constexpr void advance(RandIter& i,
    ...
    template<class InputIter>
    constexpr enable_if_t<is_input_iterator<InputIter>::value, InputIter> next(
    ...
    template<class Iter1, class Iter2>
    constexpr auto operator-(const reverse_iterator<Iter1>& x,
    ...
    template<class Ep>
    constexpr reverse_iterator<const Ep*> rbegin(initializer_list<Ep> il)
    {
        return reverse_iterator<const Ep*>(il.end());
    }

    template<class Ep>
    constexpr reverse_iterator<const Ep*> rend(initializer_list<Ep> il)
    {
        return reverse_iterator<const Ep*>(il.begin());
    }

    template<class Cp>
    constexpr auto cbegin(const Cp& c) -> decltype(faiz::begin(c))
    {
        return faiz::begin(c);
    }

    template<class Cp>
    constexpr auto cend(const Cp& c) -> decltype(faiz::end(c))
    {
        return faiz::end(c);
    }

    template<class Cp>
    constexpr auto rbegin(Cp& c) -> decltype(c.rbegin())
    {
        return c.rbegin();
    }

    template<class Cp>
    constexpr auto rbegin(const Cp& c) -> decltype(c.rbegin())
    {
        return c.rbegin();
    }

    template<class Cp>
    constexpr auto rend(Cp& c) -> decltype(c.rend())
    {
        return c.rend();
    }

    template<class Cp>
    constexpr auto rend(const Cp& c) -> decltype(c.rend())
    {
        return c.rend();
    }

    template<class Cp>
    constexpr auto crbegin(const Cp& c) -> decltype(faiz::rbegin(c))
    {
        return faiz::rbegin(c);
    }

    template<class Cp>
    constexpr auto crend(const Cp& c) -> decltype(faiz::rend(c))
    {
        return faiz::rend(c);
    }
    ...
    constexpr size_t size(const T (&)[Sz]) noexcept
    {
        return Sz;
    }
    ...
    template<class T, size_t Sz>
    constexpr bool empty(const T (&)[Sz]) noexcept
    {
        return false;
    }

    template<class Ep>
    constexpr bool empty(initializer_list<Ep> il) noexcept
    {
        return il.size() == 0;
    }
    ...
    // container.
    template<class Ep>
    constexpr const Ep* data(initializer_list<Ep> il) noexcept
    {
        return il.begin();
    }
