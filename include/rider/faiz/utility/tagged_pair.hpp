...
namespace Rider::Faiz
{
	...
	namespace _tagged_
	{
		...
		struct chain<Base, I, First, Rest...>
		{
			using type = typename First::
				template getter<Base, I, _t<chain<Base, I + 1, Rest...>>>;
		};
	...
	class tagged : public _t<_tagged_::chain<Base, 0, Tags...>>
#if RANGES_BROKEN_CPO_LOOKUP
		,
...
#endif
	{
		...
		using can_convert = bool_<!std::is_same<Other, Base>::value
	...
	public:
		...
		template<typename U,
			typename = meta::if_c<!std::is_same<tagged, decay_t<U>>::value>,
		...
		constexpr meta::if_c<range::is_swappable<B>::value>
...
#if !RANGES_BROKEN_CPO_LOOKUP
		template<typename B = Base>
		friend constexpr meta::if_c<range::is_swappable<B>::value>
	...
	get(tagged<Base, Tags...>& t)
		DECLTYPE_AUTO_RETURN_NOEXCEPT(detail::adl_get<I>(static_cast<Base&>(
			t))) template<std::size_t I, typename Base, typename... Tags>
		auto get(tagged<Base, Tags...> const& t) DECLTYPE_AUTO_RETURN_NOEXCEPT(
			detail::adl_get<I>(static_cast<Base const&>(
				t))) template<std::size_t I, typename Base, typename... Tags>
		auto get(tagged<Base, Tags...>&& t) DECLTYPE_AUTO_RETURN_NOEXCEPT(
			detail::adl_get<I>(static_cast<Base&&>(
				t))) template<std::size_t I, typename Base, typename... Tags>
	...
	auto
	get(tagged<Base, Tags...>& t)
		DECLTYPE_AUTO_RETURN_NOEXCEPT(detail::adl_get<T>(static_cast<Base&>(
			t))) template<typename T, typename Base, typename... Tags>
		auto get(tagged<Base, Tags...> const& t) DECLTYPE_AUTO_RETURN_NOEXCEPT(
			detail::adl_get<T>(static_cast<Base const&>(
				t))) template<typename T, typename Base, typename... Tags>
		auto get(tagged<Base, Tags...>&& t) DECLTYPE_AUTO_RETURN_NOEXCEPT(
			detail::adl_get<T>(static_cast<Base&&>(
				t))) template<typename T, typename Base, typename... Tags>
		void get(tagged<Base, Tags...> const&&) = delete;
	...
	template<typename Tag1,
		...
		typename T1,
		typename T2,
		typename R = tagged_pair<Tag1(range::bind_element_t<T1>),
...
\
			public: \
				...
				constexpr meta::_t<std::tuple_element<I, Untagged>> const& \
