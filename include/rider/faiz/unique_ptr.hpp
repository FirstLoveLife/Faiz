...
namespace rider::faiz
{
    ...
    struct is_safe_array_conversion
        : public faiz::integral_constant<bool,
              faiz::is_convertible_v<U_pointer, T_pointer> &&
                  faiz::is_array_v<U> &&
                  (!faiz::is_pointer_v<U_pointer> || !is_pointer_v<T_pointer> ||
                      !faiz::is_derived_of_v<T, faiz::remove_extent_t<U>>)>
    {
    };
    ...
    */
    ...
    public:
        ...
        unique_ptr& operator=(pointer p) = delete;
    ...
    public:
        ...
        unique_ptr(unique_ptr<U, E>&& u,
            faiz::enable_if_t<!faiz::is_array_v<U> &&
                    faiz::is_convertible_v<typename unique_ptr<U, E>::pointer,
                        pointer> &&
                    faiz::is_convertible_v<E, deleter_type> &&
                    (faiz::is_same_v<deleter_type, E> ||
                        !faiz::is_reference_v<deleter_type>),
                E>* = 0) noexcept
        ...
        template<typename U, typename E>
        enable_if_t<!is_array_v<U> &&
                is_convertible_v<typename unique_ptr<U, E>::pointer, pointer> &&
                faiz::is_assignable_v<deleter_type&, E&&>,
            unique_ptr&>
    ...
    public:
        ...
        unique_ptr& operator=(pointer arr) = delete;
    ...
    public:
        ...
        unique_ptr(unique_ptr<U, E>&& u,
            enable_if_t<is_safe_array_conversion<T,
                ...
                faiz::is_convertible_v<E, deleter_type> &&
                (!faiz::is_reference_v<deleter_type> ||
                    faiz::is_same_v<E, deleter_type>)>* = 0) noexcept
            : pair(u.release(), faiz::forward<E>(u.get_deleter()))
        {
        }
    ...
    template<typename T, typename D>
    inline bool operator==(const unique_ptr<T, D>& a, faiz::nullptr_t) noexcept
    {
        return !a;
    }

    template<typename T, typename D>
    inline bool operator==(faiz::nullptr_t, const unique_ptr<T, D>& a) noexcept
    {
        return !a;
    }
    ...
    template<typename T, typename D>
    inline bool operator<(const unique_ptr<T, D>& a, faiz::nullptr_t)
    {
    ...
    template<typename T, typename D>
    inline bool operator<(faiz::nullptr_t, const unique_ptr<T, D>& b)
    {
        using pointer = typename unique_ptr<T, D>::pointer;
        pointer pT = b.get();
    ...
    typename Unique_if<T>::_Single_object make_unique(Args&&... args)
    {
