...
#include "../include/rider/faiz/call_traits.hpp"
#include <algorithm>
#include <iomanip>
#include <iostream>
#include <typeinfo>

#include "boost_type_traits_tests/check_type.hpp"
#include "boost_type_traits_tests/test.hpp"
...
template<class T>
void
call_traits_checker<T>::operator()(param_type p)
{
	T t(p);
	contained<T> c(t);
	...
	cout << "typeof contained<" << typeid(T).name()
		 << ">::value() is:      " << typeid(&contained<T>::value).name()
		 << endl;
	cout << "typeof contained<" << typeid(T).name()
	...
	cout << "typeof contained<" << typeid(T).name()
		 ...
		 << endl;
	cout << "typeof contained<" << typeid(T).name()
		 << ">::call() is:       " << typeid(&contained<T>::call).name()
		 << endl;
	cout << endl;
#endif
}
...
struct call_traits_checker<T[N]>
{
	...
	operator()(param_type t)
	{
		...
		cout << "typeof contained<" << typeid(T[N]).name()
			 << ">::v_ is:         " << typeid(&contained<T[N]>::v_).name()
			 << endl;
		cout << "typeof contained<" << typeid(T[N]).name()
		...
		cout << "typeof contained<" << typeid(T[N]).name()
			 << ">::const_get is:  "
			 << typeid(&contained<T[N]>::const_get).name() << endl;
		cout << "typeof contained<" << typeid(T[N]).name()
...
void
check_wrap(const W& w, const U& u)
{
...
struct comparible_UDT
{
	...
	{}
	comparible_UDT&
	operator=(const comparible_UDT& other)
	{
		i_ = other.i_;
		return *this;
	}
	bool
	operator==(const comparible_UDT& v)
	{
		return v.i_ == i_;
	}
};
...
struct call_traits_test
{
	...
	static void
	assert_construct(param_type val);
};

template<typename T, bool isarray>
void
call_traits_test<T, isarray>::assert_construct(
