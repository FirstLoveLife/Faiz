...
test01()
{
	Rider::Faiz::unique_ptr<B[], D> b(new B[1]);
	Rider::Faiz::unique_ptr<A[], D> a(std::move(b)); //{ dg-error "no matching
													 //function" }
...
test02()
{
	Rider::Faiz::unique_ptr<A> nonarray(new A);
	Rider::Faiz::unique_ptr<A[]> array(
		std::move(nonarray)); //{ dg-error "no matching function" }
