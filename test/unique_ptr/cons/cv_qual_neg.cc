...
test07()
{
	...
	Rider::Faiz::unique_ptr<A[]> upA3(p); // { dg-error "no matching function" }
	Rider::Faiz::unique_ptr<const A[]> cA3(p); // { dg-error "no matching
											   // function" }
	Rider::Faiz::unique_ptr<volatile A[]> vA3(p); // { dg-error "no matching
												  // function" }
	Rider::Faiz::unique_ptr<const volatile A[]> cvA3(p); // { dg-error "no
														 // matching function" }
...
test08()
{
	...
	Rider::Faiz::unique_ptr<A[], deleter<A_pointer>> p;
	Rider::Faiz::unique_ptr<A[], deleter<A*>> upA(
		std::move(p)); // { dg-error "no matching function" }
}
...
test011()
{
	...
	Rider::Faiz::unique_ptr<const A[]> cA(
		std::move(upB)); // { dg-error "no matching function" }
