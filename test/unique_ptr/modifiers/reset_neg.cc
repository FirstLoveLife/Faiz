...
test01()
{
	Rider::Faiz::unique_ptr<B[]> up;
	up.reset(new A[3]); // { dg-error "no matching function" }

	Rider::Faiz::unique_ptr<A[]> up2;
...
test02()
{
	...
	Rider::Faiz::unique_ptr<A[]> upA3;
	upA3.reset(p); // { dg-error "no matching function" }
	Rider::Faiz::unique_ptr<const A[]> cA3;
	cA3.reset(p); // { dg-error "no matching function" }
	Rider::Faiz::unique_ptr<volatile A[]> vA3;
	vA3.reset(p); // { dg-error "no matching function" }
	Rider::Faiz::unique_ptr<const volatile A[]> cvA3;
